...
class HelloTriangleApplication {
 ...
 private:
  void initWindow() {
    if (!glfwInit()) std::cout << "Unable to intialize GLFW" << std::endl;
  ...
  void createLogicalDevice() {
    ...
    vkGetDeviceQueue(m_Device, indicies.graphicsFamily.value(), 0,
                     &m_GraphicsQueue);
    vkGetDeviceQueue(m_Device, indicies.presentFamily.value(), 0,
                     &m_PresentQueue);
  }
  ...
  void createGraphicsPipeline() {
    ...
    if (vkCreateGraphicsPipelines(m_Device, VK_NULL_HANDLE, 1, &pipelineInfo,
                                  nullptr, &m_GraphicPipeline) != VK_SUCCESS)
